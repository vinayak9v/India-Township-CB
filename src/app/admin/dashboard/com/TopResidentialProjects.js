...
export default function TopResidentialProjects() {
  ...
  const fetchData = async () => {
    ...
    try {
      const [projectsRes, propertiesRes] = await Promise.all([
        fetch('/api/admin/top-residential-projects').then(res => res.json()),
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const res = await fetch(`/api/admin/top-residential-projects/${id}`, { method: 'DELETE' });
      const data = await res.json();
      if (data.success) {
        fetchData();
      } else {
  ...
  return (
    <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
      <div className="px-6 py-5 border-b border-slate-100 bg-white">
        <h2 className="text-lg font-semibold text-slate-900">Top Residential Projects</h2>
        <p className="text-sm text-slate-500 mt-1">Manage top projects highlighted on the homepage.</p>
      </div>

      <div className="p-6">
        {/* Form */}
        <div className="mb-8 bg-slate-50/50 border border-slate-100 rounded-lg p-5">
          <h3 className="text-sm font-medium text-slate-800 mb-4">Add Top Project</h3>
          <form onSubmit={handleSubmit} className="flex flex-col sm:flex-row flex-wrap gap-4 items-end">
            ...
            <div className="w-full sm:w-24">
              ...
              <input type="number" value={sortOrder} onChange={(e) => setSortOrder(e.target.value)} className="w-full px-4 py-2.5 bg-white border border-slate-300 rounded-md text-sm focus:outline-none focus:border-slate-900 focus:ring-1 focus:ring-slate-900" />
            </div>

            <div className="w-full sm:w-32">
              <label className="block text-xs font-medium text-slate-700 mb-1.5">Status</label>
              <select value={status} onChange={(e) => setStatus(e.target.value)} className="w-full px-4 py-2.5 bg-white border border-slate-300 rounded-md text-sm focus:outline-none focus:border-slate-900 focus:ring-1 focus:ring-slate-900 cursor-pointer">
